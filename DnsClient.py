...
class DnsClient:
    ...
    def get_next_resource_record(self, address, dns_server_ip, req_type=package_type.A, port=53):
    ...
    def get_next_ip(self, address, dns_server_ip, req_type=package_type.A, port=53):
        request = self.request_handler.create_request(address, self.support_resolving, req_type=req_type)
        resp = self.network_handler.send_via_udp(request, dns_server_ip, port)
        self.response_handler.parse_response(resp)
    ...
    def get_ip(self, address, root_ip, req_type=package_type.A, port=53):
        if address is None:
            raise ValueError
        for answer in self.get_next_ip(address, root_ip, req_type, port):
